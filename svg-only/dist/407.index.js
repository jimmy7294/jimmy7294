...
exports.modules = {
  /***/ 407: /***/ (
    ...
    var types_grid = __webpack_require__(2881); // CONCATENATED MODULE: ./userContributionToGrid.ts
    const userContributionToGrid = (cells) => {
      ...
      for (const c of cells) {
        ...
        else (0, types_grid /* setColorEmpty */.Dy)(grid, c.x, c.y);
      }
      return grid;
    ...
    const createOutside = (grid, color = 0) => {
      const outside = (0, types_grid /* createEmptyGrid */.u1)(
        grid.width,
        grid.height,
      );
    ...
    const fillOutside = (outside, grid, color = 0) => {
      ...
      while (changed) {
        ...
        for (let x = outside.width; x--; )
          for (let y = outside.height; y--; )
            if (
              (0, types_grid /* getColor */.Lq)(grid, x, y) <= color &&
    ...
    const isOutside = (outside, x, y) =>
      !(0, types_grid /* isInside */.V0)(outside, x, y) ||
      (0, types_grid /* isEmpty */.xb)(
        (0, types_grid /* getColor */.Lq)(outside, x, y),
      );

    // EXTERNAL MODULE: ../types/snake.ts
    var types_snake = __webpack_require__(9347); // CONCATENATED MODULE: ../solver/utils/sortPush.ts
    ...
    const isEmptySafe = (grid, x, y) =>
      !(0, types_grid /* isInside */.V0)(grid, x, y) ||
      (0, types_grid /* isEmpty */.xb)(
        (0, types_grid /* getColor */.Lq)(grid, x, y),
      );
    /**
    ...
    const unwrap = (m) =>
      !m
        ...
        : [
            ...unwrap(m.parent),
            {
              x: (0, types_snake /* getHeadX */.If)(m.snake),
              y: (0, types_snake /* getHeadY */.IP)(m.snake),
            },
          ];
    /**
    ...
    const getSnakeEscapePath = (grid, outside, snake0, color) => {
      ...
      while (openList[0]) {
        ...
        for (const a of around4) {
          ...
          if (
            c <= color &&
            !(0, types_snake /* snakeWillSelfCollide */.nJ)(o.snake, a.x, a.y)
          ) {
            const snake = (0, types_snake /* nextSnake */.kv)(
              o.snake,
              a.x,
              a.y,
            );
            if (
              !closeList.some((s0) =>
                (0, types_snake /* snakeEquals */.kE)(s0, snake),
              )
            ) {
    ...
    const getBestTunnel = (grid, outside, x, y, color, snakeN) => {
      const c = { x, y };
      const snake0 = (0, types_snake /* createSnakeFromCells */.xG)(
      ...
      const snakeI = (0, types_snake /* createSnakeFromCells */.xG)(
        snakeICells,
      );
      ...
      while (openList.length) {
        ...
        for (let i = 0; i < around4.length; i++) {
          ...
          if (
            ...
            (!(0, types_grid /* isInside */.V0)(grid, nx, ny) ||
              (0, types_grid /* isEmpty */.xb)(
                (0, types_grid /* getColor */.Lq)(grid, nx, ny),
              ))
          ) {
            const nsnake = (0, types_snake /* nextSnake */.kv)(c.snake, dx, dy);
            if (
              !closeList.some((s) =>
                (0, types_snake /* snakeEquals */.kE)(nsnake, s),
              )
            ) {
              const w = c.w + 1;
      ...
      while (points.length) {
        ...
        for (const snake of path)
          clearCleanColoredLayer_setEmptySafe(
            grid,
            (0, types_snake /* getHeadX */.If)(snake),
            (0, types_snake /* getHeadY */.IP)(snake),
          );
        chain.unshift(...path);
      }
      fillOutside(outside, grid);
    ...
    const getPathToNextPoint = (grid, snake0, color, points) => {
      ...
      while (openList.length) {
        ...
        for (const { x: dx, y: dy } of around4) {
          if (
            (0, types_grid /* isInsideLarge */.HJ)(grid, 2, x + dx, y + dy) &&
    ...
    const create = (length) =>
      (0, types_snake /* createSnakeFromCells */.xG)(
    ...
    const getPathToPose = (snake0, target, grid) => {
      if ((0, types_snake /* snakeEquals */.kE)(snake0, target)) return [];
      const targetCells = (0, types_snake /* snakeToCells */.Ks)(
        target,
      ).reverse();
      const snakeN = (0, types_snake /* getSnakeLength */.JJ)(snake0);
      const box = {
        min: {
          x:
            Math.min(
              (0, types_snake /* getHeadX */.If)(snake0),
              (0, types_snake /* getHeadX */.If)(target),
            ) -
            snakeN -
            1,
          y:
            Math.min(
              (0, types_snake /* getHeadY */.IP)(snake0),
              (0, types_snake /* getHeadY */.IP)(target),
            ) -
            snakeN -
            1,
        },
      ...
      while (openList.length) {
        ...
        for (let i = 0; i < around4.length; i++) {
          ...
          if (
            ...
            if (
              !closeList.some((s) =>
                (0, types_snake /* snakeEquals */.kE)(snake, s),
              )
            ) {
              const w = o.w + 1;
              const h = Math.abs(nx - x) + Math.abs(ny - y);
          ...
          switch (format) {
            case "svg": {
              ...
              const { createSvg } = await __webpack_require__
                .e(/* import() */ 340)
                .then(__webpack_require__.bind(__webpack_require__, 8340));
              return createSvg(
                grid,
                cells,
                chain,
            ...
            case "gif": {
              ...
              const { createGif } = await Promise.all(
                /* import() */ [
                  ...
                  __webpack_require__.e(142),
                ],
              ).then(__webpack_require__.bind(__webpack_require__, 7142));
